...
export const DocumentClassification = (props: {
    classificationData?: SubmodelElementCollection[] | null;
    ...
    function extractDocumentClassificationData() {
        ...
        (props.classificationData ?? []).forEach((classificationElement) => {
        ...
        <>
            {classificationData && classificationData.length > 0 && (
                <Box>
                    ...
                    {classificationData.slice(0, 2).map((classification, index) => (
                        <Box
                            display="flex"
                            flexDirection="row"
                            gap={1}
                            key={`${classification.classId}-${classification.classificationSystem}-${index}`}
                        >
                            <Typography component="span" sx={{ mx: 1 }}>
                                •
                            </Typography>
                            {classification.classificationSystem && (
                                <Typography fontWeight="500">{classification.classificationSystem}: </Typography>
                            )}
                            <Typography>{tooltipText(classification.className, 20)}</Typography>
                        </Box>
                    ))}
