...
function getPcfSubmodelElements(submodel: Submodel): SubmodelElementCollection[] {
    ...
    const directPcfElements = findAllSubmodelElementsBySemanticIdsOrIdShort(submodel.submodelElements, null, [
    ...
    const pcfSubmodelList = findSubmodelElementBySemanticIdsOrIdShort(submodel.submodelElements, null, [
...
function extractLifeCyclePhaseValue(element: SubmodelElementCollection): string | null {
    const lifeCycleElement = findSubmodelElementBySemanticIdsOrIdShort(element.value, null, [
...
function extractCalculationMethod(pcfSubmodelElements: SubmodelElementCollection[]): string {
    for (const element of pcfSubmodelElements) {
        const methodElement = findSubmodelElementBySemanticIdsOrIdShort(element.value, null, [
...
function extractCO2EquivalentsPerLifeCycleStage(
    ...
    pcfSubmodelElements.forEach((element) => {
        ...
        const stage =
            (lifeCyclePhaseValue?.split(' ')[0].trim() as ProductLifecycleStage) ?? ProductLifecycleStage.A3Production;

        const co2Value = findSubmodelElementBySemanticIdsOrIdShort(element.value, null, [
            PcfSubmodelElementSemanticIdEnum.PCFCO2eq,
...
function extractTotalCO2Equivalents(pcfSubmodelElements: SubmodelElementCollection[]): number {
    return pcfSubmodelElements.reduce((total, element) => {
        const co2Element = findSubmodelElementBySemanticIdsOrIdShort(element.value, null, [
            PcfSubmodelElementSemanticIdEnum.PCFCO2eq,
            PcfSubmodelElementSemanticIdEnum.PCFCO2eqV1,
        ]) as Property;
...
function extractAddressPerLifeCyclePhase(element: SubmodelElementCollection, locale: string): AddressPerLifeCyclePhase {
    const lifeCyclePhaseValue = extractLifeCyclePhaseValue(element);
    const lifeCyclePhase = lifeCyclePhaseValue?.split(' ')[0].trim() as ProductLifecycleStage;

    const addressHandover = findSubmodelElementBySemanticIdsOrIdShort(element.value, null, [
    ...
    const addressData = {
        latitude: findValueByIdShort(
            addressElements,
            null,
            PcfSubmodelElementSemanticIdEnum.PCFAddressLatitude,
            locale,
        ),
        longitude: findValueByIdShort(
            addressElements,
            null,
            PcfSubmodelElementSemanticIdEnum.PCFAddressLongitude,
            locale,
        ),
        rawStreet:
            findValueByIdShort(addressElements, null, PcfSubmodelElementSemanticIdEnum.PCFAddressStreet, locale) ||
            findValueByIdShort(addressElements, null, SubmodelElementSemanticIdEnum.ContactInformationStreet, locale),
        rawHouseNumber: findValueByIdShort(
            addressElements,
            null,
            PcfSubmodelElementSemanticIdEnum.PCFAddressHouseNumber,
            locale,
        ),
        zipCode:
            findValueByIdShort(addressElements, null, PcfSubmodelElementSemanticIdEnum.PCFAddressZipCode, locale) ||
            findValueByIdShort(addressElements, null, SubmodelElementSemanticIdEnum.ContactInformationZipCode, locale),
        cityTown:
            findValueByIdShort(addressElements, null, PcfSubmodelElementSemanticIdEnum.PCFAddressCityTown, locale) ||
            findValueByIdShort(addressElements, null, SubmodelElementSemanticIdEnum.ContactInformationCityTown, locale),
        country:
            findValueByIdShort(addressElements, null, PcfSubmodelElementSemanticIdEnum.PCFAddressCountry, locale) ||
            findValueByIdShort(
                addressElements,
                null,
                SubmodelElementSemanticIdEnum.ContactInformationNationalCode,
                locale,
            ),
    };
...
function extractStreetAndHouseNumber(
