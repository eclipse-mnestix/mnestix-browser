...
import { HierarchicalStructuresSubmodelElementSemanticId } from 'app/[locale]/viewer/_components/submodel/hierarchical-structures/HierarchicalStructuresSubmodelElementSemanticId.enum';
...
export function HierarchicalStructuresDetail({ submodel }: SubmodelVisualizationProps) {
    ...
    const archeTypePropertylElement = smElements.find((el) => {
        if (
            ...
            el.semanticId?.keys[0].value === HierarchicalStructuresSubmodelElementSemanticId.ArcheType
        ) {
...
function separateEntryNode(smElements: Entity[]) {
    const entryNode = smElements.find((el) => {
        return el.semanticId?.keys[0].value === HierarchicalStructuresSubmodelElementSemanticId.EntryNode;
    });
...
function isBulkCountProperty(el: ISubmodelElement) {
    return el.semanticId?.keys[0]?.value === HierarchicalStructuresSubmodelElementSemanticId.BulkCount;
}
