import { getSubmodelFromSubmodelDescriptor } from './submodelRegistryActions';
...
const DEFAULT_REPO: RepositoryWithInfrastructure = {
    url: 'http://backend:8081/submodels/s',
    infrastructureName: 'Default Infrastructure',
};

describe('getSubmodelFromSubmodelDescriptor', () => {
    ...
    beforeEach(() => {
        ...
        fetchMock.mockResolvedValue({ isSuccess: true, result: { id: 's' } });
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        mockedMnestixFetch.mockReturnValue({ fetch: fetchMock } as any);
    });
    ...
    it('attaches no credentials (null header) for a non-configured host', async () => {
        ...
        await getSubmodelFromSubmodelDescriptor(repository);
    ...
    it('attaches the security header for a configured host and returns the result', async () => {
        ...
        const result = await getSubmodelFromSubmodelDescriptor(DEFAULT_REPO);
