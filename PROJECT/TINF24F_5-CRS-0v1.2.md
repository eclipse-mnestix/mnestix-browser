...
| 1.2     | 20.03.2026 | Julian Schumacher | Split requirements into groups and categorize with specific IDs |
...
5. [Functional Requirements](#5-functional-requirements)
   5.1 [Login (LOGI)](#51-login-logi) \
   5.2 [UI/UX (UI)](#52-uiux-ui) \
   5.3 [AAS List (LIST)](#53-aas-list-list) \
   5.4 [Nameplate Generator (NPG)](#54-nameplate-generator-npg) \
   5.5 [Linked AAS (LINKED)](#55-linked-aas-linked) \
   5.6 [Performance (PERFORM)](#56-performance-perform) \
   5.7 [Configuration (CONFIG)](#57-configuration-config) \
   5.8 [eShop (SHOP)](#58-eshop-shop) \
   5.9 [Language Support (SHOP-LANG)](#59-language-support-shop-lang)
...
| -------------- | ---------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| LOGI-FR-001    | The system shall display the user login status permanently in the header section.  | Provides immediate authentication feedback and maintains a visible login indicator throughout the session. |
| LOGI-FR-002    | The new login button shall take over all the features of the existing login button | All features, including menus, error handling etc. shall be preserved.                                     |
...
| Requirement ID | Short Description                                           | Details                                                                                                                                                                               |
| -------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| UI-FR-001      | The number of AAS entries per repository shall be displayed | in the repository overview, there shall be a number indicating the count of AAS entries available in each repository.                                                                 |
| UI-FR-002      | The `SMTechnicalData` formatting shall be improved          | The `SMTechnicalData` submodel shall be presented in a structured layout with consistent column widths, readable formatting, and clear section labels for better readability.         |
| UI-FR-003      | The `SMHandoverDocumentation` formatting shall be improved  | The `SMHandoverDocumentation` submodel shall be presented in a structured layout with consistent column widths, readable formatting, and clear section labels for better readability. |
...
| Requirement ID | Short Description                                                             | Details                                                                                                                                                                                                           |
| -------------- | ----------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| LIST-FR-001    | The system shall display multiple columns in the AAS list                     | for better overview and usability, the AAS list shall display multiple columns, including ManufacturerName, ProductDesignation, OrderCode, ManufacturerCode, globalAssetId and createdAt.                         |
| LIST-FR-002    | The system shall allow users to filter the AAS list based on query parameters | The AAS list shall support filtering based on the attributes ManufacturerName, ProductDesignation, OrderCode, ManufacturerCode, globalAssetId and createdAt via URL parameters.                                   |
| LIST-FR-003    | The system shall allow users to sort the AAS list by any available column     | The AAS list shall support sorting based on the attributes ManufacturerName, ProductDesignation, OrderCode, ManufacturerCode, globalAssetId and createdAt via columns or query parameters as specified in FR-005. |

---

### 5.4 Nameplate Generator (NPG)

| Requirement ID | Short Description                                                        | Details                                                                                                                                                                                                                       |
| -------------- | ------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| NPG-FR-001     | The Nameplate Generator shall be integrated into the product detail view | The Nameplate Generator shall be accessible via the context menu in the product detail view, using the fiven implementation repository, which can be found [here](https://github.com/ttrsf/TINF22F-Team2-Nameplate-Generator) |

---

### 5.5 Linked AAS (LINKED)

| Requirement ID | Short Description                                                              | Details                                                                                         |
| -------------- | ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------- |
| LINKED-FR-001  | Navigating through linked AAS references should be possible (Bill of material) | The system shall allow navigation through linked AAS references within submodel visualizations. |

---

### 5.6 Performance (PERFORM)

| Requirement ID | Short Description                                                                                                  | Details                                                                                    |
| -------------- | ------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------ |
| PERFORM-FR-001 | The product list view shall retrieve AAS data asynchronously to avoid blocking the interface.                      | Ensures a non-blocking interface and optimized performance, especially on mobile networks. |
| PERFORM-FR-002 | The product list view shall load thumbnails and other media content lazily, restricted to visible items on screen. | Loads only visible media elements to conserve data and improve performance.                |

---

### 5.7 Configuration (CONFIG)

| Requirement ID | Short Description                                                            | Details                                                                                                                          |
| -------------- | ---------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| CONFIG-FR-001  | The system shall allow users to enable or disable individual CD repositories | Users shall be able to enable or disable specific CD repositories through the configuration view without removing them entirely. |
| CONFIG-FR-002  | Users shall be able to inspect the content of CD repositories                | The system shall allow users to view the content of CD repositories through the user interface.                                  |
...
| SHOP-FR-004    | The system shall provide the option to add products to the cart               | In the product detail view, the system shall provide an "Add to Cart" button that allows users to add the currently viewed product to their cart.                           |
| SHOP-FR-005    | The sidebar shall indicate the total number of products in the cart           | The sidebar shall display a cart icon with a badge or number indicating the total number of products currently in the cart.                                                 |
| SHOP-FR-006    | The eShop System shall be disableable via configuration                       | The eShop functionality shall be optional and can be enabled or disabled via the `.env` file with the flag `SHOP_ENABLED_FLAG`.                                             |
| SHOP-FR-007    | The system shall support integration with an external payment gateway         | The eShop system shall be designed to allow future integration with external payment gateways for processing transactions.                                                  |
| SHOP-FR-008    | The system shall display product prices                                       | In the product overview there shall be a price attribute displayed for each product, which is also reflected in the cart view, if the eShop module is active.               |
| SHOP-FR-009    | The system shall allow users to enable or disable individual AAS repositories | Users shall be able to enable or disable specific AAS repositories through the configuration view without removing them entirely.                                           |
| SHOP-FR-010    | Accessible via `/cart` route                                                  | The shopping cart feature shall be accessible via the `/cart` route of the system                                                                                           |
| SHOP-FR-011    | implement required data structures                                            | Required data structures for the card feature shall be added. Already existing data structure shall be used where applicable                                                |
| SHOP-FR-012    | caching cart content locally                                                  | The shopping cart feature shall store the current content of the cart using the local storage mechanism                                                                     |
| SHOP-FR-013    | Menu Bar Icon incl. badge                                                     | A new icon shall be added to the menu bar to navigate to the cart and indicate the current number of items in the cart                                                      |
| SHOP-FR-014    | Navigating via menu bar icon                                                  | clicking the menu bar icon shall navigate to the `/cart` route specified in FR-024                                                                                          |
| SHOP-FR-015    | Realtime update                                                               | The cart and its content shall update in realtime                                                                                                                           |
| SHOP-FR-016    | No empty card badge                                                           | When the cart is empty the badge defined in FR-027 shall not be displayed                                                                                                   |
| SHOP-FR-017    | Cart overview                                                                 | The shopping cart view (which access is defined in FR-024) shall display a preview of items including multiple relevant information such as product name, quanity and price |
| SHOP-FR-018    | total price display                                                           | The total price of the cart shall be displayed in a sum at the bottom of the view                                                                                           |
| SHOP-FR-019    | remove items from shop                                                        | the user shall be provided with the option to remove each item in the cart individually                                                                                     |
| SHOP-FR-020    | adjust item count                                                             | the item count per item in the cart shall be adjustable by using a numberic input field                                                                                     |
| SHOP-FR-021    | checkout (preperation)                                                        | the cart shall implement a "checkout" button to procede to checkout. The actual checkout is not within the scope of this implementation.                                    |
| SHOP-FR-022    | empty cart message                                                            | If the cart is empty, a message stating that the cart is empty shall be displayed                                                                                           |
| SHOP-FR-023    | responsive design                                                             | the shopping cart view shall be responsive to work on both mobile and desktop                                                                                               |
| SHOP-FR-024    | add product button                                                            | within the scope of this implementation, a "add to cart" button shall be implemented into the product detail view to add a product to the cart                              |
| SHOP-FR-025    | navigate to cart option after adding procuct                                  | after adding a product to the cart the user shall be provided with the option to either navigate to the cart of stay on the current page                                    |
| SHOP-FR-026    | disable shopping cart feature                                                 | the shopping cart feature shall provide a flag to disable the use of this feature                                                                                           |
| SHOP-FR-027    | configurable external checkout path                                           | the url to the custom checkout handler shall be configurable via config files                                                                                               |

---

### 5.9 Language Support (SHOP-LANG)

| Requirement ID   | Short Description                | Details                                                                                         |
| ---------------- | -------------------------------- | ----------------------------------------------------------------------------------------------- |
| SHOP-LANG-FR-001 | multilangual support             | the shopping cart feature shall be available in at least english, german and spanish            |
| SHOP-LANG-FR-002 | using existing translation files | the translations for the shopping cart feature shall be added to the existing translation files |
